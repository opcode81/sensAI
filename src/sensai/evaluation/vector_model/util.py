...
from typing import Generic, Optional, Dict, Any, Union, Sequence, Callable, Tuple, TypeVar
...
from ...models.vector_model import VectorModel, VectorRegressionModel, VectorClassificationModel
...
def _isRegression(model: Optional[VectorModel], isRegression: Optional[bool]):
...
def getVectorModelEvaluator(data, model: VectorModel = None, isRegression: bool = None, **kwargs) \
        -> Union[VectorRegressionModelEvaluator, VectorClassificationModelEvaluator] :
    if _isRegression(model, isRegression):
        return VectorRegressionModelEvaluator(data, **kwargs)
    else:
        return VectorClassificationModelEvaluator(data, **kwargs)


def getVectorModelCrossValidator(data, model: VectorModel = None, isRegression: bool = None, folds=5, **kwargs) \
...
def evalModelViaEvaluator(model: VectorModel, inputOutputData: InputOutputData, testFraction=0.2,
    ...
    :param normalizePlots: whether to to normalize plotted distributions such that they sum to 1
    :param randomSeed:
...
class EvaluationUtil(ABC, Generic[TModel, TEvaluator, TEvalData, TCrossValData, TEvalStats]):
    ...
    # TODO: This is somehow asymmetric, as there is no createCrossValidator. Do we need this method?
    def createEvaluator(self, model) -> TEvaluator:
        ...
        :return: an evaluator
        """
        return getVectorModelEvaluator(self.inputOutputData, model=model, **self.evaluatorParams)
    ...
    def performCrossValidation(self, model: TModel, showPlots=False, logResults=True,
                               resultWriter: Optional[ResultWriter] = None) -> TCrossValData:
        """
        ...
        crossValidator = getVectorModelCrossValidator(self.inputOutputData, model=model, **self.crossValidatorParams)
    ...
    # TODO: seems like the functionality related to predictedVarName disappeared, is it still desired?
    def createPlots(self, data: Union[TEvalData, TCrossValData], showPlots=True,
                resultWriter: Optional[ResultWriter] = None, subtitlePrefix: str = ""):
        """
...
class MultiDataEvaluationUtil:
    ...
    def compareModelsCrossValidation(self, isRegression, modelFactories: Sequence[Callable[[], VectorModel]],
            ...
            crossValidatorParams: Optional[Dict[str, Any]] = None, columnNameForModelRanking: str = None, rankMax=True) \
...
# TODO or not TODO: the amount of generics makes it hard to read, maybe we can reduce the complexity somehow?
class RegressionEvaluationUtil(EvaluationUtil[VectorRegressionModel, VectorRegressionModelEvaluator,
                                              VectorRegressionModelEvaluationData, VectorRegressionModelCrossValidationData,
                                              RegressionEvalStats]):
...
class ClassificationEvaluationUtil(EvaluationUtil[VectorClassificationModel, VectorClassificationModelEvaluator,
                                                  VectorClassificationModelEvaluationData, VectorClassificationModelCrossValidationData,
